...
class Mesh:
    ...
    def extrude_face(self, face_index: int, distance: float = 0.1) -> "Mesh":
        ...
        if fi < 0 or fi >= self.face_count:
            return self
        idx = _face_indices(self.faces[fi])
        if len(idx) < 3:
            return self
        p0 = self._pos(self.vertices[idx[0]])
        p1 = self._pos(self.vertices[idx[1]])
        p2 = self._pos(self.vertices[idx[2]])
        u = Vector3(p1.x - p0.x, p1.y - p0.y, p1.z - p0.z)
        v = Vector3(p2.x - p0.x, p2.y - p0.y, p2.z - p0.z)
        n = _normalize(Vector3(u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z, u.x * v.y - u.y * v.x))
        if _length(n) <= 1e-9:
            n = Vector3(0.0, 1.0, 0.0)
        new_loop: List[int] = []
        for vi in idx:
            p = self._pos(self.vertices[int(vi)])
            new_loop.append(self.add_vertex((p.x + n.x * distance, p.y + n.y * distance, p.z + n.z * distance)))
        self.add_face(new_loop)
        for i in range(len(idx)):
            a = idx[i]
            b = idx[(i + 1) % len(idx)]
            c = new_loop[(i + 1) % len(new_loop)]
            d = new_loop[i]
    ...
    def scale_face(self, face_index: int, scale: float = 1.0) -> "Mesh":
        fi = int(face_index)
        if fi < 0 or fi >= self.face_count:
            return self
        idx = _face_indices(self.faces[fi])
        ...
        s = float(scale)
        for vi in idx:
            vtx = self.vertices[int(vi)]
            p = self._pos(vtx)
            self._set_pos(vtx, Vector3(c.x + (p.x - c.x) * s, c.y + (p.y - c.y) * s, c.z + (p.z - c.z) * s))
        return self.calculate_normals()

    def translate_face(self, face_index: int, delta: VecLike) -> "Mesh":
        fi = int(face_index)
        if fi < 0 or fi >= self.face_count:
            return self
        d = _vec3(delta)
        ...
        for vi in idx:
            vtx = self.vertices[int(vi)]
            p = self._pos(vtx)
            self._set_pos(vtx, Vector3(p.x + d.x, p.y + d.y, p.z + d.z))
        return self.calculate_normals()
    ...
    def inset_faces(self, amount: float = 0.05) -> "Mesh":
        ...
        for fi in list(self.selected_faces) or [0]:
            if hasattr(self._cpp_mesh, "scale_face"):
                ...
                continue
            if hasattr(self._cpp_mesh, "scaleFace"):
                self._cpp_mesh.scaleFace(int(fi), float(scale))
                continue
            self.scale_face(int(fi), scale)
        return self.calculate_normals()
    ...
    def subdivide(self, levels: int = 1) -> "Mesh":
        ...
        for _ in range(max(1, int(levels))):
            faces_src = [_face_indices(f) for f in self.faces]
            if not faces_src:
                break
            old_points = [(self._pos(v).x, self._pos(v).y, self._pos(v).z) for v in self.vertices]
            new_points: List[Tuple[float, float, float]] = list(old_points)
            ...
            def midpoint(a: int, b: int) -> int:
                ...
                pa = old_points[a]
                pb = old_points[b]
                idx_mid = len(new_points)
                new_points.append(((pa[0] + pb[0]) * 0.5, (pa[1] + pb[1]) * 0.5, (pa[2] + pb[2]) * 0.5))
                edge_mid[key] = idx_mid
                return idx_mid
            ...
            for face in faces_src:
                if len(face) < 3:
                    continue
                center = (
                    sum(old_points[i][0] for i in face) / len(face),
                    sum(old_points[i][1] for i in face) / len(face),
                    sum(old_points[i][2] for i in face) / len(face),
                )
                ci = len(new_points)
    ...
    def brush(self, brush_type: str, center: VecLike, radius: float, strength: float = 0.1, falloff: str = "SMOOTH", direction: VecLike = (0.0, 1.0, 0.0)) -> "Mesh":
        ...
        if b in ("inflate", "draw", "clay", "clay strips", "blob", "fill") and hasattr(self._cpp_mesh, "inflate"):
            ...
            return self
        if b in ("inflate", "draw", "clay", "clay strips", "blob", "fill"):
            rad = max(1e-9, float(radius))
            for i, v in enumerate(self.vertices):
                if self.mask_weights.get(i, 0.0) >= 1.0:
                    continue
                p = self._pos(v)
                off = Vector3(p.x - c.x, p.y - c.y, p.z - c.z)
                d = _length(off)
                if d > rad:
                    continue
                n = _normalize(off) if d > 1e-9 else Vector3(0.0, 1.0, 0.0)
                w = self._falloff(d, rad, falloff) * float(strength) * (1.0 - self.mask_weights.get(i, 0.0))
                self._set_pos(v, Vector3(p.x + n.x * w, p.y + n.y * w, p.z + n.z * w))
            return self.calculate_normals()
        d = _vec3(direction)
    ...
    def sculpt_filter(self, filter_type: str = "smooth", strength: float = 0.25, radius: Optional[float] = None) -> "Mesh":
        ...
        nbrs: Dict[int, Set[int]] = {i: set() for i in range(self.vertex_count)}
        for a, b in edges:
            nbrs[a].add(b)
            nbrs[b].add(a)
        old = [self._pos(v) for v in self.vertices]
        c = Mesh.cursor()
        rad = float(radius) if radius is not None else None
        s = max(0.0, min(1.0, abs(float(strength))))
        for i, v in enumerate(self.vertices):
            if self.mask_weights.get(i, 0.0) >= 1.0 or not nbrs[i]:
                continue
            p = old[i]
            if rad is not None and _length(Vector3(p.x - c.x, p.y - c.y, p.z - c.z)) > rad:
                continue
            sx = sy = sz = 0.0
            for j in nbrs[i]:
                q = old[j]
                ...
                sz += q.z
            inv = 1.0 / len(nbrs[i])
            avg = Vector3(sx * inv, sy * inv, sz * inv)
            self._set_pos(v, Vector3(p.x + (avg.x - p.x) * s, p.y + (avg.y - p.y) * s, p.z + (avg.z - p.z) * s))
        return self.calculate_normals()
    ...
    def merge_duplicate_vertices(self, tolerance: float = 1e-6) -> "Mesh":
        ...
        faces: List[List[int]] = []
        for f in self.faces:
            idx = [remap[int(i)] for i in _face_indices(f)]
            clean: List[int] = []
            for i in idx:
